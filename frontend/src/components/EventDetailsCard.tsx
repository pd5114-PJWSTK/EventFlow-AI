...
import type { EventItem, EventRequirementItem, ListResponse, LocationItem } from "../types/api";
...
function requirementText(requirement: EventRequirementItem): string {
  ...
  if (requirement.requirement_type === "equipment_type") return `${quantity} x equipment type ${String(requirement.equipment_type_id || "").slice(0, 8)}`;
  if (requirement.requirement_type === "vehicle_type") return `${quantity} x ${humanize(requirement.vehicle_type_required)} vehicle`;
  if (requirement.requirement_type === "person_skill") return `${quantity} x skill ${String(requirement.skill_id || "").slice(0, 8)}`;
...
export function EventDetailsCard({ eventId, title = "Selected event" }: EventDetailsCardProps): JSX.Element | null {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      ...
      const [locationData, requirementData] = await Promise.all([
  ...
  const requirementSummary = useMemo(() => requirements.map(requirementText), [requirements]);
          ...
          <>
            ...
            <Stack spacing={0.75}>
              ...
              {requirementSummary.length === 0 ? (
                <Typography color="text.secondary">No requirements saved for this event.</Typography>
              ) : (
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                  {requirementSummary.map((item) => <Chip key={item} label={item} />)}
                </Stack>
              )}
            </Stack>
          </>
