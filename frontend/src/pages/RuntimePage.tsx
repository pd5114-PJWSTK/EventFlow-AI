...
import { Alert, Box, Button, Checkbox, Chip, FormControlLabel, Grid, Paper, Stack, TextField, Typography } from "@mui/material";
...
export function RuntimePage(): JSX.Element {
  ...
  return (
    <Stack spacing={2.5}>
      ...
      {replanResult && <Paper variant="outlined" sx={{ p: 3, borderRadius: 2, position: "relative" }}><Stack spacing={2}><BackCornerButton onClick={() => setReplanResult(null)} /><Typography variant="h6">Replan recommendation</Typography><Typography>{replanResult.comparison.decision_note}</Typography><Typography color="text.secondary">New cost: {formatMoney(replanResult.comparison.new_cost)}. Cost change: {formatMoney(replanResult.comparison.cost_delta)}.</Typography><Paper variant="outlined" sx={{ p: 2, borderRadius: 2, bgcolor: "rgba(15, 118, 110, 0.05)" }}><Stack spacing={1}><Typography fontWeight={800}>Operator action plan</Typography><Typography variant="body2" color="text.secondary">Review what is already done and change the remaining actions before approving the replan.</Typography>{actions.map((action, index) => <Grid container spacing={1.5} key={index} alignItems="center"><Grid item xs={12} md={2}><FormControlLabel control={<Checkbox checked={action.status === "done"} onChange={(event) => setActions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, status: event.target.checked ? "done" : "pending" } : item))} />} label={action.status === "done" ? "Done" : "Pending"} /></Grid><Grid item xs={12} md={6}><TextField label="Action" value={action.label} onChange={(event) => setActions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item))} fullWidth /></Grid><Grid item xs={12} md={3}><TextField label="Owner" value={action.owner} onChange={(event) => setActions((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, owner: event.target.value } : item))} fullWidth /></Grid><Grid item xs={12} md={1}><Button color="warning" onClick={() => setActions((current) => current.filter((_item, itemIndex) => itemIndex !== index))}>Remove</Button></Grid></Grid>)}<Button variant="outlined" onClick={() => setActions((current) => [...current, { label: "Add an operational step.", owner: "Coordinator", status: "pending" }])}>Add action</Button></Stack></Paper><Stack direction="row" spacing={1}><Button variant="contained" color="success" disabled={isWorking} onClick={() => void acceptReplan()}>Approve</Button><Button variant="outlined" color="warning" onClick={() => resetFlow("The replan was rejected and the view was reset.")}>Reject</Button></Stack></Stack></Paper>}
    </Stack>
  );
}
